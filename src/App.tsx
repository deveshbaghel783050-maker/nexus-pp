...
export default function App() {
  ...
  useEffect(() => {
    const cachedCart = localStorage.getItem('pp_cart');
    if (cachedCart) {
      try {
    ...
    const cachedBookings = localStorage.getItem('pp_bookings');
    if (cachedBookings) {
      try {
  ...
  const handleAddBooking = (newBooking: TableBooking) => {
    ...
    localStorage.setItem('pp_bookings', JSON.stringify(updated));
  };
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-brand-outline leading-relaxed font-sans max-w-xs">
              © 2024 pp Mumbai. Handcrafted using eco-conscious Mumbai local clay and slow artisanal materials.
            </p>
